...
from quiz.quiz_view import QuizView
...
class Quiz(commands.Cog):
    ...
    async def quizz(
        ...
        public: Option(bool, "If the quizz is public or not", required=False, default=True) # type: ignore
        ):
        if channel == '*':
            if channel_category == '*':
        ...
        if id is None:
            await ctx.respond("I couldn't find any URLs in the messages. channel: " + channel.mention, ephemeral=True)
        else:
            await ctx.respond("**Here is some random music**", view=QuizView(id, content), ephemeral=not public)
